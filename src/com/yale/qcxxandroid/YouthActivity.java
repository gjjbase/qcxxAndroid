		...
		public void onReceive(Context context, Intent intent) {
			...
			try {
				...
				Dao<XmppMsgBean, Integer> messageDao = DataHelper.getInstance(
						YouthActivity.this).getXmppMsgDAO();
				Where<XmppMsgBean, Integer> query = messageDao.queryBuilder()
						.where().eq("chatTopic", name).and()
						.eq("isReaded", false);
		...
		try {
			...
			if (results != null && results.size() > 0) {
				for (int i = results.size(); i > 0; i--) {
					...
					Where<XmppMsgBean, Integer> query = messageDao
							.queryBuilder().where()
							.eq("chatTopic", msg.getFromUserId()).and()
							.eq("isReaded", false).and().eq("msgtype", 1);
					List<XmppMsgBean> list = messageDao.query(query.prepare());
					msgCount = list.size();
					tmp.put("tongzhi", msgCount);
					jsonList.add(tmp);
